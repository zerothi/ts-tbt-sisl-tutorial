...
discouraged by the length of the code, there are only $7$ lines of actual code.
...
encouraged to do this after you have completed this example). Secondly, you should also
reduce \script{run.py} from 2 \code{construct} calls to 1 \code{construct} call
(see bottom of \script{run.py}).
...
calculation is based on a skewed unit-cell\footnote{For those with experience in the older
    ...
    directions to be orthogonal to the other lattice vectors. This is true for \emph{both}
    \ts\ and \tbt.}.
...
spectrum. Note that this example uses the Hamiltonian tiling.
